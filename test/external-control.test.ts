...
describe('External Control Tests', () => {
  ...
  describe('Configuration', () => {
    it('should include external control paths in schema', () => {
      ...
      expect(plugin.schema.properties.externalUpPath.title).to.include('External Up Control Path')
      expect(plugin.schema.properties.externalDownPath.title).to.include('External Down Control Path')
    })
    ...
    it('should start with external control paths configured', () => {
      ...
      const subscribedPaths = subscription.options.subscribe.map((s: any) => s.path)
  ...
  describe('Chain Counter with External Control', () => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              const chainMessages = mockApp.messages.filter(m => 
                m.message?.updates?.[0]?.values?.[0]?.path === 'navigation.anchor.chainOut'
              )
              console.log('Up chain messages:', chainMessages.map(m => m.message.updates[0].values[0].value))
    ...
    it('should not interfere with relay-controlled windlass PUT handlers', (done) => {
      const windlassHandler = mockApp.putHandlers.get(externalConfig.windlassPath)
      expect(windlassHandler).to.exist
      
      // External control should not affect PUT handler registration
      const result = windlassHandler('vessels.self', externalConfig.windlassPath, 'up', (callbackResult: any) => {
        expect(callbackResult.state).to.equal('COMPLETED')
        setTimeout(() => {
          expect(mockApp.relayStates.get(externalConfig.upRelayPath)).to.be.true
          
          // Check that windlass state path reflects the relay up state
          const windlassStateMessage = mockApp.messages.find(m => 
            m.message?.updates?.[0]?.values?.[0]?.path === externalConfig.windlassPath &&
  ...
  describe('Windlass State Path Updates', () => {
    ...
    it('should update windlass state path when external up control is active', (done) => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const offStateMessage = mockApp.messages.find(m => 
            m.message?.updates?.[0]?.values?.[0]?.path === externalConfig.windlassPath &&
    ...
    it('should prioritize external control state over relay control state', (done) => {
      ...
      setTimeout(() => {
        ...
        let windlassStateMessages = mockApp.messages.filter(m => 
          m.message?.updates?.[0]?.values?.[0]?.path === externalConfig.windlassPath
        )
        expect(windlassStateMessages[windlassStateMessages.length - 1].message.updates[0].values[0].value).to.equal('up')
        ...
        setTimeout(() => {
          ...
          const externalStateMessage = mockApp.messages.find(m => 
            m.message?.updates?.[0]?.values?.[0]?.path === externalConfig.windlassPath &&
            m.message?.updates?.[0]?.values?.[0]?.value === 'down'
          )
          ...
          setTimeout(() => {
            ...
            const revertStateMessage = mockApp.messages.find(m => 
              m.message?.updates?.[0]?.values?.[0]?.path === externalConfig.windlassPath &&
              m.message?.updates?.[0]?.values?.[0]?.value === 'up'
            )
